...
namespace Mineral.Core.Capsule.Util
{
    ...
    public class RLP
    {
        ...
        public static readonly byte[] EMPTY_ELEMENT_RLP = EncodeElement(new byte[0]);
            ...
            int endPosition, int levelToIndex, RLPCollection rlpCollection)
        {
            ...
            catch (System.Exception ex)
            {
                throw new System.Exception(
                    "Invalid RLP " + currentData.ToHexString(), ex);
            }
        }

        public static LList DecodeLazyList(byte[] data)
        {
            LList list = DecodeLazyList(data, 0, data.Length);
            return list == null ? null : list.GetList(0);
        }
        ...
        public static LList DecodeLazyList(byte[] data, int pos, int length)
        {
            ...
            while (pos < end)
            {
                ...
                {
                    result.Add(pos, 0, false);
                    pos++;
                }
                else if (prefix < OFFSET_SHORT_ITEM)
                {
                    result.Add(pos, 1, false);
                    pos++;
                }
                else if (prefix <= OFFSET_LONG_ITEM)
                {
                    int len = prefix - OFFSET_SHORT_ITEM;
                    result.Add(pos + 1, len, false);
                    pos = len + 1;
                }
                else if (prefix < OFFSET_SHORT_LIST)
                {
                    int len = prefix - OFFSET_LONG_ITEM;
                    int len_bytes = ByteArrayToInt(data.SubArray(pos + 1, pos + 1 + len));
                    VerifyLength(len_bytes, data.Length - pos - 1 - len);
                    result.Add(pos + 1 + len, len_bytes, false);
                    pos += 1 + len + len_bytes;
                }
                else if (prefix <= OFFSET_SHORT_LIST)
                {
                    int len = prefix - OFFSET_SHORT_LIST;
                    result.Add(pos + 1, len, true);
                    pos += 1 + len;
                }
                else if (prefix <= 0xFF)
                {
                    int len = prefix - OFFSET_LONG_LIST;
                    int len_list = ByteArrayToInt(data.SubArray(pos + 1, pos + 1 + len));
                    VerifyLength(len_list, data.Length - pos + 1 + len);
                    result.Add(pos + 1 + len, len_list, true);
                    pos += 1 + len + len_list;
                }
                else
                {
                    throw new System.Exception("Only byte values between 0x00 and 0xFF are supported, but got : " + prefix);
                }
            }
        ...
        private static void VerifyLength(int supplied, int available)
        {
            if (supplied > available)
                throw new System.Exception(
                    string.Format("Length parsed from RLP ({0} bytes) is greater"
                     + "than possible size of data ({1} bytes)"
                     , supplied
                     , available
                    ));
        }
