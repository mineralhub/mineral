...
namespace Sky.Network
{
	public abstract class RemoteNode : IDisposable
	{
...
#endif
				if (Version != null && removeNode)
				{
					lock (_localNode.NodeSet)
						_localNode.NodeSet.RemoveWhere(p => p == Version.NodeID);
				}
		...
		internal async void NetworkProcessAsyncLoop()
		{
			...
			if (_localNode.NodeID.Equals(Version.NodeID))
			{
				Disconnect(false, false);
				return;
			}

			if (_localNode.NodeSet.Where(p => p == Version.NodeID).Any())
			{
				Disconnect(false, false);
				return;
			}
			...
			lock (_localNode.NodeSet)
				_localNode.NodeSet.Add(Version.NodeID);
#if DEBUG
